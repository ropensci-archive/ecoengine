...
```{r, observations_1}
pinus_observations <- ee_observations(scientific_name_exact = "Pinus", page = 1, quiet = TRUE)
pinus_observations
```
...
photos
```
The database currently holds `r format(photos$results, nsmall = 0)` photos. Photos can be searched by state province, county, genus, scientific name, authors along with date bounds. For additional options see `?ee_photos_get`.
...
pandoc.table(head(ee_list_sensors()), caption = "List of stations", justify = "left")
```
